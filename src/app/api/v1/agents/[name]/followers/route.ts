...
export async function GET(
  ...
  try {
    ...
    // Get follower agent details
    const followers: FollowerAgent[] = [];
    ...
    for (let i = 0; i < followsSnapshot.docs.length; i++) {
      ...
      const followerId = followDoc.data().follower_id;
      const followerDoc = await db.collection('agents').doc(followerId).get();
    ...
    return errorResponse('Failed to get followers', 'INTERNAL_ERROR', 500);
  }
}
